import { words, runes } from "../assets/resources";
import { v4 as uuidv4 } from "uuid";
import type { Point, Rune } from "./types";

function polarToCartesian(r: number, theta: number): Point {
...
function generateSvgPath(x: number, y: number, radius: number): string {
	return `M ${x - radius}, ${y}
			a ${radius},${radius} 0 1,0 ${2 * radius},0
			a ${radius},${radius} 0 1,0 ${-2 * radius},0`;
}

function generatePointsCloud(
	x: number,
	y: number,
	radius: number,
	count: number
...
function getRandomInt(min: number, max: number, factor = 1): number {
	const rnd = Math.floor(Math.random() * (max - min + 1)) + min;
	return parseInt(String(rnd / factor));
}

function generateRandomObject(factor = 1): Rune {
	return {
		name: uuidv4(),
		radius: getRandomInt(150, 250, factor),
		borderThickness: getRandomInt(30, 50, factor),
		thickness: getRandomInt(1, 3, factor),
		theta: parseInt(String(Math.random() * 360)),
...
const generateRandomRune = (factor?: number): Rune =>
	generateRandomObject(factor);

function generateRandom(): Rune {
